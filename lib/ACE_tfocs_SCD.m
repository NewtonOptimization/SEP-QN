function [ x, odata, opts ] = ACE_tfocs_SCD( objectiveF, affineF, dualproxF, mu, x0, z0, opts, contOpts )
...
if iscell(objectiveF)
    ...
    if length(mu)>1
      ...
      smoothF = @(varargin)my_smooth_dual( objectiveF, mu, x0, varargin{:} );
    end
end

% ----------------------- Subfunctions ----------------------------------------

function [ prox, x ] = my_smooth_dual( objectiveF, mu, x0, ATz )
% Adding 0 to ATz will destroy the sparsity
if (isscalar(x0) && x0 == 0) || numel(x0) == 0 || nnz(x0) == 0
    [ v, x ] = objectiveF( ATz, mu);
else
    [ v, x ] = objectiveF( mu * x0 + ATz, mu );
end
prox = tfocs_dot( ATz, x ) - v - (0.5*mu) * tfocs_normsq( x - x0 );
prox = -prox;
x = -x;
