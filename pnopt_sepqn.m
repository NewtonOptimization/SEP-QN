function [ x, f_x, output ] = pnopt_sep_qn( smoothF, affineF, dualproxF, x, z0, nonsmoothF, rho, options )
    ...
    if iter > 1
        ...
        switch subprob_solver
            ...
            case 'bfgs-tfocs'
                ...
                %                     max( 0.0001 * optim_tol, forcing_term * optim ));
                %                 tfocs_opts.errFcn =  @(f,x) tfocs_err() ;
                sf = smooth_quad( H, grad_g_x-(H*x), 0 );
                
                tfocs_opts.continuation = 1;
                
                %c_opts = {'maxIts', 10, 'tol', '1e-8'};
                [ x_prox, tfocs_out ] = ...
                    tfocs_SCD( sf, affineF, dualproxF, rho, x, z0, tfocs_opts);
            ...
            case 'lbfgs-tfocs'
                ...
                for cont=1:1
                   ...
                   % tfocs_opts.Lexact = 1 / rho;
                    sf = my_smooth_quad(sPrev, Ninv_s, Ninv_y, D, YS, SS, SNY, SNS, grad_g_x-H_x(x), hDiag, H_x);
                    [ x_prox, tfocs_out ] = ...
                        ACE_tfocs_SCD( sf, affineF, dualproxF, rho, x_prox, z0, tfocs_opts);
                    ...
                    rho = rho * rhoDecrement;                    
                end
                
                tfocs_opts.tol = ftol;
                tfocs_opts.maxIts = 200;
                % tfocs_opts.Lexact = Inf;
                [Ninv_s, Ninv_y, D, SNS, SNY] = quasi_bfgs_update(sPrev, yPrev, YS, SS, 0, hDiag);
                sf = my_smooth_quad(sPrev, Ninv_s, Ninv_y, D, YS, SS, SNY, SNS, grad_g_x-H_x(x), hDiag, H_x);
                [ x_prox, tfocs_out ] = ...
                    ACE_tfocs_SCD( sf, affineF, dualproxF, 0, x_prox, z0, tfocs_opts);
                if iscell(tfocs_out.dual)
                    z0 = cell( tfocs_out.dual );
                else
                    z0 = tfocs_out.dual;
